...
out_path = workingDir + '//outputSHP//' +  shpFolder + '//{0}.shp'
...
with open(inputCSV, 'rb') as csvfile:
    ...
    for i,row in enumerate(r):
        if i > 0 and row[8] in systemArgs: #skip header
            ...
            tempDate = row[13].split('/')
            dateShpFormat = str(tempDate[2]) + str(tempDate[0]).zfill(2) + str(tempDate[1]).zfill(2)
            #print(dateShpFormat);
            #MATT_HANSEN_DEFOR.append(row[12])
            PROBABILITY.append(row[12])
            DATE.append(dateShpFormat)
...
for j,k in enumerate(x):
    ...
    w.record(k,y[j],UNIQUE_ID[j], RES[j], TILEH[j], TILEV[j], COL[j], ROW[j], ISO3[j], PERC_TREE_COVER[j], ADM_REGION[j], ECO_REGION[j], PROBABILITY[j], DATE[j]) #write the attributes MATT_HANSEN_DEFOR[j], 
