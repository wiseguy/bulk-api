...
function removeOldData() {
    try {
        var result1 = findRemoveSync(outCSVFolder, {
            age: {
                seconds: 172800
            },
            files: '*.*',
        ...
        var result2 = findRemoveSync(outFGDBfolder, {
            age: {
                seconds: 172800
            },
            files: '*.*',
        ...
        var result3 = findRemoveSync(outSHPfolder, {
            age: {
                seconds: 172800
            },
            files: '*.*',
        ...
        var result4 = findRemoveSync(outKMLfolder, {
            age: {
                seconds: 172800
            },
            files: '*.*',
        ...
        var result5 = findRemoveSync(outZip, {
            age: {
                seconds: 172800
            },
            files: '*.*',
...
function executeDownload(response, data) {
    ...
    var stream = collection.find(queryObj, {
        "PROBABILITY": {
            $slice: [startDateIndex, numberOfDates]
        }
    }).limit(limit || 0).sort({
        ISO3: 1
    }).stream();
    ...
    var fanoutCountry = ["ALL"];

    if (!fanout) {
        currentFile = outCSVFolder + "\\ALL_" + outCSVFilename;
        fs.appendFileSync(currentFile, csvColumns);
    } else {
        fanoutCountry = [];
    }
    ...
    stream.on('data', function(data) {
        ...
        //callback(users[0]);
        if (fanout && fanoutCountry.indexOf(data.ISO3) < 0) {
            console.log(data.ISO3); //new country
            //create file if needed
            if (fanoutCountry.length > 1) {
                endOfDataBlock(currentFile, fanoutCountry[fanoutCountry.length - 1]); //previous country
            }

            currentFile = outCSVFolder + "\\" + data.ISO3 + "_" + outCSVFilename;
            fs.appendFileSync(currentFile, csvColumns);

            fanoutCountry.push(data.ISO3);
        }
        ...
        for (var k = 0; k < numberOfDates; k++) {
            ...
            if (currentProb < minProb || currentProb > maxProb) {
                ...
                var insertRowCSV = "\n" + unique_id + "," + (data.RES || 0) + "," + (data.TILEH || 0) + "," + (data.TILEV || 0) + "," + (data.COL || 0) + "," + (data.ROW || 0) + "," + (data.LAT || 0.0) + "," + (data.LON || 0.0) + "," + (data.ISO3 || "NULL") + "," + (data.PERC_TREE_COVER || 0) + "," + (data.ADM_REGION || 0) + "," + (data.ECO_REGION || 0) + "," + (data.PROBABILITY[k] || 0) + "," + parseInt(startDateIndex + k) + "," + dateIndexes[startDateIndex + k].toString(); //data.DATE

                //console.log("Saved Prob " + currentProb + " for " + unique_id);

                //Add Row to CSV
                //console.log(currentFile);
                fs.appendFileSync(currentFile, insertRowCSV);
                if (thresholdProb && currentProb >= thresholdProb) {
                    foundThreshold = true;
                }
    ...
    stream.on('end', function() {
        ...
        endOfDataBlock(currentFile, fanoutCountry[fanoutCountry.length - 1]);


    });

    var endOfDataBlock = function(fileName, countryName) {
        ...
        switch (output) {
            ...
            case "csv":
                ...
                var zipCmd = '7z a "' + outZipFile + '" "' + fileName + '"';
                ...
                //HANDLE GDB
            case "gdb":
                var gdbFolder = "FORMA_GDB_" + countryName + "_" + random;
                ...
                var exportLine = 'python "' + __dirname + '\\exportgdbfix.py" "' + __dirname + '" "' + fileName + '" ' + random + ' ' + countryName;
            ...
            case "shp":
                ...
                var exportLine = 'python "' + __dirname + '\\exportshpfix.py" "' + __dirname + '" "' + fileName + '" ' + random + ' ' + iso3;
